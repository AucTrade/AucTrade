...
class AuctionServiceImplTest {
    ...
    @Test
    @DisplayName("경매 시작 시간이 현재 시간보다 이전일 경우 WRONG_AUCTION_STARTAT 에러 메시지 발생")
    ...
    @Test
    @DisplayName("경매 등록 성공시 저장한 경매 ID와 경매 성공 여부 ( true )를 반환한다.")
    void createAuction() {
    ...
    void getAllMyBeforeStartedAuctionsWhenMyBeforeStartedExists() {
        ...
        assertEquals(1, result.getAuctions().size(),
                "내가 생성한 시작전 경매는 1건 이어야 합니다.");

        assertEquals(myAuctions.get(myAuctions.size()-1).getId(), result.getAuctions().get(0).getId(),
                "조회된 경매 ID가 일치하지 않습니다.");
    }
    @Test
    ...
    void getAllMyBeforeStartedAuctionsWhenMyStartedExists() {
        ...
        assertEquals(myAuctions.get(1).getId(), result.getAuctions().get(0).getId(),
                "조회된 경매 ID가 일치하지 않습니다.");
    }
    ...
    void getAllMyBeforeStartedAuctionsWhenMyEndedExists() {
        ...
        assertEquals(myAuctions.get(0).getId(), result.getAuctions().get(0).getId(),
                "조회된 경매 ID가 일치하지 않습니다.");
    }
    ...
    void getMaxParticipation() {
        ...
        assertEquals(auction.getMaxParticipants(), auctionService.getMaxParticipation(auction.getId()),
                "조회한 최대 인원수가 일치하지 않습니다.");
    }
    ...
    void getMinimumPrice() {
        ...
        assertEquals(auction.getMinimumPrice(), auctionService.getMinimumPrice(auction.getId()),
                "조회한 최소 입찰금이 일치하지 않습니다.");
    }
    ...
    void getStartAt() {
        ...
        assertEquals(formatter.format(auction.getStartAt()), auctionService.getStartAt(auction.getId()).substring(0, 23),
                "조회한 최소 입찰금이 일치하지 않습니다.");
    }
    ...
    void placeDepositAtStartedAuction() {
        ...
        CustomException exception = assertThrows(CustomException.class,
                () -> auctionService.placeDeposit(new AuctionDto.PutDeposit(3500), auction.getId(), user.getEmail()));
    ...
    void placeDepositInvalidAmount() {
        ...
        CustomException exception = assertThrows(CustomException.class,
                () -> auctionService.placeDeposit(new AuctionDto.PutDeposit(1000), auction.getId(), user.getEmail()));
    ...
    void placeDeposit() {
        ...
        AuctionDto.Result result =  auctionService.placeDeposit(new AuctionDto.PutDeposit(amount), auction.getId(), user.getEmail());
    ...
    void placeDepositInvalidUpdateAmount() {
        ...
        depositSet.remove(user.getEmail());

        int amount = 4000;
        String email = user.getEmail();
        auctionService.placeDeposit(new AuctionDto.PutDeposit(amount), auction.getId(), email);
    ...
    @Test
    @DisplayName("이전에 등록한 예치금보다 높은 금액 수정 요청 시 성공 여부 및 해당 경매 id 반환")
    void placeDepositUpdateAmount() {
        User user = generateUser();
        user.addPoint(50000);
        ...
        AuctionDto.Result result = auctionService.placeDeposit(new AuctionDto.PutDeposit(amount), auction.getId(), email);
    ...
    @Test
    @DisplayName("예치금 등록 취소 요청 시 성공 여부 및 해당 경매 id 반환")
    void cancelDeposit() {
        User user = generateUser();
        user.addPoint(50000);
        ...
        auctionService.placeDeposit(new AuctionDto.PutDeposit(4000), auction.getId(), user.getEmail());

        AuctionDto.Result result = auctionService.cancelDeposit(auction.getId(), user.getEmail());
        assertEquals(auction.getId(), result.getAuctionId());
        assertTrue(result.getIsSuccess());
    }
